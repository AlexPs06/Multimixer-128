...
void Multimixer(const uint8_t* input, uint8_t* tag, uint8x16_t *keys, const uint64_t lenght);
...
/**
 * @brief Update function based on vector additions and multiplications.
 *
 * This function computes auxiliary vectors U and V derived from the
 * partial sums of X and Y, respectively. It then performs pairwise
 * 32-bit multiplications using NEON instructions, accumulating the
 * 64-bit results into the output buffer.
 *
 * @param X      Input vector of four 32-bit unsigned integers.
 * @param Y      Input vector of four 32-bit unsigned integers.
 * @param output Accumulator array of four 128-bit vectors (uint64x2_t),
 *               where each entry stores two 64-bit partial results.
 */
static inline void update_function(uint32x4_t X,
                                   uint32x4_t Y,
                                   uint64x2_t *output)
{
    /* Initialize auxiliary vectors U and V to zero */
    uint32x4_t U = vdupq_n_u32(0);
    uint32x4_t V = vdupq_n_u32(0);

    /*
     * Compute U and V as modified sums of X and Y.
     *
     * For U:
     *   U[i] = sum(X) - X[(i + 3) mod 4]
     *
     * For V:
     *   V[i] = sum(Y) - Y[i]
     *
     * vaddvq_u32 performs a horizontal addition over the 4 lanes.
     */
    for (int i = 0; i < 4; i++) {
        U[i] = vaddvq_u32(X) - X[(i + 3) % 4];
        V[i] = vaddvq_u32(Y) - Y[i];
    }

    /*
     * Split input vectors into low and high 32-bit halves.
     * This is required because vmull_u32 operates on uint32x2_t
     * and produces a uint64x2_t result.
     */
    uint32x2_t low_X  = vget_low_u32(X);   // X[0], X[1]
    uint32x2_t high_X = vget_high_u32(X);  // X[2], X[3]

    uint32x2_t low_Y  = vget_low_u32(Y);   // Y[0], Y[1]
    uint32x2_t high_Y = vget_high_u32(Y);  // Y[2], Y[3]

    uint32x2_t low_U  = vget_low_u32(U);   // U[0], U[1]
    uint32x2_t high_U = vget_high_u32(U);  // U[2], U[3]

    uint32x2_t low_V  = vget_low_u32(V);   // V[0], V[1]
    uint32x2_t high_V = vget_high_u32(V);  // V[2], V[3]

    /*
     * Perform pairwise 32-bit multiplications:
     *   - X * Y  (low and high halves)
     *   - U * V  (low and high halves)
     *
     * Each vmull_u32 produces two 64-bit products packed
     * into a uint64x2_t vector.
     */
    uint64x2_t result[4];
    result[0] = vmull_u32(low_X,  low_Y);   // X[0]*Y[0], X[1]*Y[1]
    result[1] = vmull_u32(high_X, high_Y);  // X[2]*Y[2], X[3]*Y[3]
    result[2] = vmull_u32(low_U,  low_V);   // U[0]*V[0], U[1]*V[1]
    result[3] = vmull_u32(high_U, high_V);  // U[2]*V[2], U[3]*V[3]

    /*
     * Accumulate the newly computed products into the output buffer.
     * Each output entry acts as a running accumulator of 64-bit results.
     */
    for (int i = 0; i < 4; i++) {
                 ...
                 const uint64_t lenght)
{
    ...
    for (i = 0; i < size - 1; i = i + 2) {
        ...
        /*
         * Addition input blocks with the generated masks and
         * reinterpret them as 32-bit word vectors.
         */
        uint32x4_t X =
            vaddq_s32(vreinterpretq_u32_u8(block_x), vreinterpretq_u32_u8(generate_key_x));
        uint32x4_t Y =
            vaddq_s32(vreinterpretq_u32_u8(block_y), vreinterpretq_u32_u8(generate_key_y)); 

        
        

        /*
         * Update the internal hash state using integer
